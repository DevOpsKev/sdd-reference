...
# sdd/helloworld, sdd/homepage/header). That directory must contain spec.md;
# provenance.md and scenarios.md live alongside it.
#
# Inputs:  SPEC (required), e.g. sdd/vite-baseline or ./sdd/homepage
...
resolve_spec_dir() {
  ...
  case "$d" in
    *..*)
    ...
    sdd/*) ;;
    *)
      echo "SPEC must be a repo-relative directory under sdd/, e.g. sdd/helloworld (got: $SPEC)" >&2
      return 1
      ;;
  esac
