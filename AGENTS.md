...
**`sdd/`** holds SDD artefacts only. No other paths under `sdd/` may be modified by the agent (specs and sibling inputs remain human-authored unless the spec says otherwise). That includes **`sdd/context/**`**, **`sdd/reference/**`**, **`sdd/agents/**`**, and **`sdd/scripts/**`** — shared background, fixed references, and infrastructure only; do not add, edit, or delete files there. Do not add extra files under the active spec directory beyond **`provenance.md`** and **`scenarios.md`** as allowed for the role (both are Markdown audit outputs only), and do not modify other **`sdd/specs/**`** spec trees.
...
- Generated implementation files belong at the repo root (or wherever the spec directs); committed automated tests belong under **`e2e/`**, not under **`sdd/`**
