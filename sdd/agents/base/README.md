...
under `sdd/agents/<name>/`. Files here are **read at runtime**
from the streamed workspace (`/work/sdd/agents/base/`); they are
...
| `lib/spec-paths.sh` | every `run-*.sh` | `resolve_spec_dir` — normalizes **`SPEC`** (repo-relative spec directory under **`sdd/specs/`**) and exports `SPEC_PATH`, provenance/scenarios paths, `SPEC_SLUG`. |
...
2. Each agent's `Dockerfile` build context stays scoped to its own
   directory (`sdd/agents/<name>/`). Adding a `COPY` from a
...
- `sdd/agents/` is in the agent no-modify list (see `AGENTS.md`).
  Agents may **read** from `sdd/agents/base/` at runtime — that
